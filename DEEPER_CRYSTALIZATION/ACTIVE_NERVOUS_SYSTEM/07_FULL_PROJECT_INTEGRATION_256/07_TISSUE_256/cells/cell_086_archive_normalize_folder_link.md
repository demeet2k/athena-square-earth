...
# Cell 086 - archive.normalize.folder.link

## Local law

Use `normalize` on the `Archive` body at `folder` scale until it can `connect the representation to parents, peers, and downstream destinations`.
...
Touch one concrete source under `archive_atlas.json`, fold it through `normalize` at `folder` scale, and land the result in a witness-bearing bridge toward `self_actualize` that can `connect the representation to parents, peers, and downstream destinations`.
