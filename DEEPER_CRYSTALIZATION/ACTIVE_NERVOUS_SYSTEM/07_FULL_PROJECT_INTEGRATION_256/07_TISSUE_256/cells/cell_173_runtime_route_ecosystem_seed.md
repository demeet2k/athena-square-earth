...
# Cell 173 - runtime.route.ecosystem.seed

## Local law

Use `route` on the `Runtime` body at `ecosystem` scale until it can `define the minimum admissible representation and stop it from disappearing again`.
...
Touch one concrete source under `self_actualize`, fold it through `route` at `ecosystem` scale, and land the result in a witness-bearing bridge toward `ACTIVE_NERVOUS_SYSTEM` that can `define the minimum admissible representation and stop it from disappearing again`.
