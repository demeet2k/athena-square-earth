...
# Cell 206 - manuscript.intake.ecosystem.link

## Local law

Use `intake` on the `Manuscript` body at `ecosystem` scale until it can `connect the representation to parents, peers, and downstream destinations`.
...
Touch one concrete source under `ACTIVE_NERVOUS_SYSTEM`, fold it through `intake` at `ecosystem` scale, and land the result in a witness-bearing bridge toward `DEEPER CRYSTALIZATION` that can `connect the representation to parents, peers, and downstream destinations`.
