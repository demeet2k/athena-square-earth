...
# Cell 244 - manuscript.replay.file.publish

## Local law

Use `replay` on the `Manuscript` body at `file` scale until it can `promote the route into a canonical surface other agents can safely inherit`.
...
Touch one concrete source under `ACTIVE_NERVOUS_SYSTEM`, fold it through `replay` at `file` scale, and land the result in a witness-bearing bridge toward `DEEPER CRYSTALIZATION` that can `promote the route into a canonical surface other agents can safely inherit`.
