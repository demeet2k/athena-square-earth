...
# Cell 170 - runtime.route.framework.link

## Local law

Use `route` on the `Runtime` body at `framework` scale until it can `connect the representation to parents, peers, and downstream destinations`.
...
Touch one concrete source under `self_actualize`, fold it through `route` at `framework` scale, and land the result in a witness-bearing bridge toward `ACTIVE_NERVOUS_SYSTEM` that can `connect the representation to parents, peers, and downstream destinations`.
