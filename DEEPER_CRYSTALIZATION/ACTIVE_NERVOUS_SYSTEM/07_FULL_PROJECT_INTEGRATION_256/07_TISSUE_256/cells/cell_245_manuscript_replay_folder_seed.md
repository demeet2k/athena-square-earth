...
# Cell 245 - manuscript.replay.folder.seed

## Local law

Use `replay` on the `Manuscript` body at `folder` scale until it can `define the minimum admissible representation and stop it from disappearing again`.
...
Touch one concrete source under `ACTIVE_NERVOUS_SYSTEM`, fold it through `replay` at `folder` scale, and land the result in a witness-bearing bridge toward `DEEPER CRYSTALIZATION` that can `define the minimum admissible representation and stop it from disappearing again`.
