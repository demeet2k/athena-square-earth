...
# Cell 079 - archive.intake.ecosystem.prove

## Local law

Use `intake` on the `Archive` body at `ecosystem` scale until it can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
...
Touch one concrete source under `archive_atlas.json`, fold it through `intake` at `ecosystem` scale, and land the result in a witness-bearing bridge toward `self_actualize` that can `add witness, receipt, metric, or replay so the route becomes trustworthy`.
