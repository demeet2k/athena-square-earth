...
# Improvement Step 3130

## Improvement Signature

- `recursion` -> `atlas` -> `integrate` -> `seed`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `atlas` into `integrate` and `seed`.
