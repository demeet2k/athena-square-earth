...
# Improvement Step 0011

## Improvement Signature

- `density` -> `control` -> `expand` -> `relay`
...
This step repairs a local weakness where `density` has not yet been fully carried through `control` into `expand` and `relay`.
