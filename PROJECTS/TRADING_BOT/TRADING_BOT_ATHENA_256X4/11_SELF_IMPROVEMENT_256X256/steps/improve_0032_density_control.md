...
# Improvement Step 0032

## Improvement Signature

- `density` -> `control` -> `integrate` -> `audit`
...
This step repairs a local weakness where `density` has not yet been fully carried through `control` into `integrate` and `audit`.
