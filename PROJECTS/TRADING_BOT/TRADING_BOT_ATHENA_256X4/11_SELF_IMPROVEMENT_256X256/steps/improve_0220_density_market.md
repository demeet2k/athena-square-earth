...
# Improvement Step 0220

## Improvement Signature

- `density` -> `market` -> `verify` -> `seed`
- role owner: `R08 Asset Librarian`
- metro route: `L8 Deployment Line` via `H8 Audit Hub` -> `H2 Phase Hub`
- chapter anchor: `Ch02 Corpus Intake and Gaps`
- appendix anchor: `AppH Risk Schema`

## Gap Addressed

This step repairs a local weakness where `density` has not yet been fully carried through `market` into `verify` and `seed`.
