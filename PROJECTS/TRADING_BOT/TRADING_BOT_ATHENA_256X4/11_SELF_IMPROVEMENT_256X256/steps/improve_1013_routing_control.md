...
# Improvement Step 1013

## Improvement Signature

- `routing` -> `control` -> `expand` -> `renew`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `expand` and `renew`.
