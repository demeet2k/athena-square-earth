...
# Improvement Step 1113

## Improvement Signature

- `routing` -> `atlas` -> `expand` -> `renew`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `atlas` into `expand` and `renew`.
