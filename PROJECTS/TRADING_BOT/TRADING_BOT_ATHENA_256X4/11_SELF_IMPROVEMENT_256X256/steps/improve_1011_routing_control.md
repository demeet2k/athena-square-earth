...
# Improvement Step 1011

## Improvement Signature

- `routing` -> `control` -> `expand` -> `relay`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `expand` and `relay`.
