...
# Improvement Step 3132

## Improvement Signature

- `recursion` -> `atlas` -> `integrate` -> `audit`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `atlas` into `integrate` and `audit`.
