...
# Improvement Step 1012

## Improvement Signature

- `routing` -> `control` -> `expand` -> `audit`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `expand` and `audit`.
