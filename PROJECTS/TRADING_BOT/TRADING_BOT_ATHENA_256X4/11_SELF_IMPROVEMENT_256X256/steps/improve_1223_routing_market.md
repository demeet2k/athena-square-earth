...
# Improvement Step 1223

## Improvement Signature

- `routing` -> `market` -> `verify` -> `renew`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `market` into `verify` and `renew`.
