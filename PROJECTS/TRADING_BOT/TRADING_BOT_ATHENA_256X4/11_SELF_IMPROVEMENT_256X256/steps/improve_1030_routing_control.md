...
# Improvement Step 1030

## Improvement Signature

- `routing` -> `control` -> `integrate` -> `seed`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `integrate` and `seed`.
