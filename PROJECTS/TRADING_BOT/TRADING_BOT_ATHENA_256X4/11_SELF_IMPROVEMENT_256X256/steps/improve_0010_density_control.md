...
# Improvement Step 0010

## Improvement Signature

- `density` -> `control` -> `expand` -> `seed`
...
This step repairs a local weakness where `density` has not yet been fully carried through `control` into `expand` and `seed`.
