...
# Improvement Step 3012

## Improvement Signature

- `recursion` -> `control` -> `expand` -> `audit`
- role owner: `R12 Portfolio Balancer`
- metro route: `L4 Market Line` via `H9 Deployment Hub` -> `H3 Asset Hub`
- chapter anchor: `Ch06 Asset Universe Cartography`
- appendix anchor: `AppL Evidence Plans`

## Gap Addressed

This step repairs a local weakness where `recursion` has not yet been fully carried through `control` into `expand` and `audit`.
