...
# Improvement Step 1020

## Improvement Signature

- `routing` -> `control` -> `verify` -> `seed`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `verify` and `seed`.
