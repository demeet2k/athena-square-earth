...
# Improvement Step 3013

## Improvement Signature

- `recursion` -> `control` -> `expand` -> `renew`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `control` into `expand` and `renew`.
