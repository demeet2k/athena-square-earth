...
# Improvement Step 1232

## Improvement Signature

- `routing` -> `market` -> `integrate` -> `audit`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `market` into `integrate` and `audit`.
