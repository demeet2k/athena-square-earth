...
# Improvement Step 0102

## Improvement Signature

- `density` -> `atlas` -> `repair` -> `audit`
...
This step repairs a local weakness where `density` has not yet been fully carried through `atlas` into `repair` and `audit`.
