...
# Improvement Step 3022

## Improvement Signature

- `recursion` -> `control` -> `verify` -> `audit`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `control` into `verify` and `audit`.
