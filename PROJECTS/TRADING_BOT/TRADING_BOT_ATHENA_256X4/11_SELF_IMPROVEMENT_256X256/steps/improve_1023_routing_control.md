...
# Improvement Step 1023

## Improvement Signature

- `routing` -> `control` -> `verify` -> `renew`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `control` into `verify` and `renew`.
