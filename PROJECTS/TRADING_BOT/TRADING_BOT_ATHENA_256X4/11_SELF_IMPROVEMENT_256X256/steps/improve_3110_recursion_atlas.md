...
# Improvement Step 3110

## Improvement Signature

- `recursion` -> `atlas` -> `expand` -> `seed`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `atlas` into `expand` and `seed`.
