...
# Improvement Step 3211

## Improvement Signature

- `recursion` -> `market` -> `expand` -> `relay`
- role owner: `R11 Execution Broker`
- metro route: `L3 Kernel Line` via `H1 Intake Hub` -> `H4 Signal Hub`
- chapter anchor: `Ch04 Kernel Algebra`
- appendix anchor: `AppK Conflict Ledger`

## Gap Addressed

This step repairs a local weakness where `recursion` has not yet been fully carried through `market` into `expand` and `relay`.
