...
# Improvement Step 3222

## Improvement Signature

- `recursion` -> `market` -> `verify` -> `audit`
...
This step repairs a local weakness where `recursion` has not yet been fully carried through `market` into `verify` and `audit`.
