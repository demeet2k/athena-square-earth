...
# Improvement Step 1330

## Improvement Signature

- `routing` -> `governance` -> `integrate` -> `seed`
...
This step repairs a local weakness where `routing` has not yet been fully carried through `governance` into `integrate` and `seed`.
