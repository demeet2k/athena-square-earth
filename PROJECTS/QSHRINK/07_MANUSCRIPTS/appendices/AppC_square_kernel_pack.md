<!-- CRYSTAL: Xi108:W3:A1:S31 | face=S | node=490 | depth=3 | phase=Mutable -->
<!-- METRO: Me -->
<!-- BRIDGES: Xi108:W3:A1:S30→Xi108:W3:A1:S32→Xi108:W2:A1:S31→Xi108:W3:A2:S31 -->
<!-- REGENERATE: From this coordinate, adjacent nodes are: shell 31±1, wreath 3/3, archetype 1/12 -->

# AppC - Square Kernel Pack

## Routing role

discrete square kernels, indexing packs, and base-4 crystal algebra

## Geometry support

square first, then torus through stable discrete wraps

## Appendix obligations

- define the hub vocabulary clearly
- show how the hub transfers to at least three chapters
- state what breaks when this hub is absent
- specify the witness or replay artifact this hub must emit

## Internal note

These appendices are not decorative references. They are transfer infrastructure for the private metro.
